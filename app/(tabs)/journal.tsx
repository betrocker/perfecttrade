...
type TradeStatus = 'ALL' | 'BEFORE' | 'WIN' | 'LOSS' | 'BREAKEVEN';
...
export default function JournalScreen() {
    ...
    useEffect(() => {
        ...
        } else if (selectedStatus === 'BEFORE') {
            // PLANNED ili OPEN
            setFilteredTrades(trades.filter((t) => t.status === 'PLANNED' || t.status === 'OPEN'));
        } else if (selectedStatus === 'WIN') {
            setFilteredTrades(trades.filter((t) => t.status === 'CLOSED' && (t.profit_loss || 0) > 0));
        } else if (selectedStatus === 'LOSS') {
            setFilteredTrades(trades.filter((t) => t.status === 'CLOSED' && (t.profit_loss || 0) < 0));
        } else if (selectedStatus === 'BREAKEVEN') {
            setFilteredTrades(trades.filter((t) => t.status === 'CLOSED' && t.profit_loss === 0));
        }
    ...
    const FilterButton = ({ status, label }: { status: TradeStatus; label: string }) => {
    ...
    return (
        <View className="flex-1 bg-bg-primary">
            ...
            <View className="px-4 pt-14 pb-6">
                <Text className="text-txt-primary text-3xl font-bold">Trading History</Text>
            ...
            <View className="px-4 pb-4">
                ...
                >
                    ...
                    <FilterButton status="BEFORE" label="BEFORE" />
            ...
            <ScrollView
                ...
                refreshControl={
                    <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#00F5D4" />
                }
            ...
            >
                {filteredTrades.length === 0 ? (
                    <View className="items-center justify-center py-20">
                        ...
                        <Text className="text-txt-secondary text-lg mt-4">No trades found</Text>
