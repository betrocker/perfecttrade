...
// Tip za Supabase odgovor
type SupabaseResponse<T> = {
  data: T | null;
  error: PostgrestError | null;
};

// Utility funkcija za retry logiku sa timeout-om
...
class DashboardService {
  async getDashboardStats(userId: string): Promise<DashboardStats> {
    try {
      const { data: trades, error } = await fetchWithRetry<Trade[]>(
        async () => {
  ...
  async getTradingDays(
    ...
    try {
      ...
      if (__DEV__) {
        console.error("Trading days error:", error);
      }
      return {};
    }
  }
  ...
  async getPerformingPairs(
    ...
    try {
      ...
      if (__DEV__) {
        console.error("Performing pairs error:", error);
      }
      return { best: null, worst: null };
    }
  }
